...
def create_app(engine: ChargeEngine, static_dir: Path = _STATIC_DIR) -> web.Application:
    app = web.Application()
...
async def handle_get_settings(request: web.Request) -> web.Response:
    ...
    return web.json_response({
        "charge_limit": cfg.limit,
        "allowed_drop": cfg.allowed_drop,
        "pd_mode": cfg.pd_mode,
    })
...
async def handle_put_settings(request: web.Request) -> web.Response:
    ...
    try:
        engine.update_config(
            ...
            allowed_drop=data.get("allowed_drop"),
...
async def _handle_ws_message(engine: ChargeEngine, ws: web.WebSocketResponse, raw: str):
    ...
    if msg_type == "set_limit":
        try:
            engine.update_config(limit=data.get("value"))
        except (ValueError, TypeError) as e:
